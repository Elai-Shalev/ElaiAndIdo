...
public class ItamarTester {
    ...
    public static boolean isAVLTree(IAVLNode node) {
        ...
        if (!(node.getSize() == getTreeSize(node))){
            j=1;
        }
        ...
        if (!(!node.getRight().isRealNode() || node.getRight().getKey() > node.getKey())){
            j = 1;
        }
        if (!(!node.getLeft().isRealNode() || node.getLeft().getKey() < node.getKey())){
            j = 1;
        }
